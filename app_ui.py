"""
Streamlit UI for Pesticide Recommendation Chatbot (API Client)
"""
...
# Configuration
API_BASE_URL = "http://localhost:8000"  # Change this if API is hosted elsewhere
...
with st.sidebar:
    ...
    # Action Buttons
    if st.button("🗑️ Clear Chat", use_container_width=True, disabled=not api_online):
    ...
    st.caption("Built with ❤️ using AWS Bedrock & FastAPI")
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤔 Thinking..."):
            try:
                ...
                # Show session state update if changed
